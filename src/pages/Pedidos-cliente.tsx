...
import { Pedido } from "@/lib/pedidos";
...
export default function Pedidos_cliente() {
  ...
  useEffect(() => {
    if (cpf) {
      getPedidosCliente(cpf).then((dados) => setPedidos(dados));
    }
  }, [cpf]);
  ...
  async function getPedidosCliente (cpf:string) {
    try {
      const res = await api.get<Pedido[]>("/pedidos/"+cpf);
